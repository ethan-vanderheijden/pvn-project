...
def _create_ports(count, network):
    body = {"ports": []}
...
def _create_container(port, image):
    result = config.zun.containers.create(image=image, nets=[{"port": port}], auto_remove=True)
    uuid = result.to_dict()["uuid"]
    if osc_utils.wait_for_status(config.zun.containers.get, uuid, success_status=["created"]):
        return uuid
    else:
...
def _start_pvn(pvn_id, pvn_json):
    try:
        ports = _create_ports(len(pvn_json["apps"]), cfg.CONF.network.id)
